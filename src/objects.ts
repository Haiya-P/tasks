...
export function isCorrect(question: Question, answer: string): boolean {
    ...
    let trim_lower = lower.trim();
    return question.expected.toLowerCase().trim() == trim_lower;
}
...
export function toShortForm(question: Question): string {
    let this_question = question.id;
    let characters = question.name.slice(0, 10);
    return this_question + ": " + characters;
}
